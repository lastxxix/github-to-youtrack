...
export class SyncService {
    ...
    public async syncRepo(mapping: RepoMapping): Promise<void> {
        ...
        for (const issue of issues) {
            ...
            if (!existingIssue) {
                ...
                if (issue.comments > 0) {
                const comments = await gh.fetchComments(
                    githubRepo, 
                    issue.number, 
                    existingIssue ? lastUpdate : undefined
                );
    ...
    public async performInitialMigration(mappings: RepoMapping[]): Promise<void> {
